...
//		printOrderList();
		//	logger.tell(msg, ActorRef.noSender());
//		System.out.println("****");
...
//			System.out.println(getSender().path().name()+" tell to symbol actor " +this.name +" OrderId "+orderMsg.order.getOrderId());
			currentOrder = orderMsg.order;
		    ProcessOrder(orderMsg);
		}
		else{
			System.out.println("Unkonwn MSG Msg.subject="+Msg.subject.toString());
		}
//		printOrderList();
 	}
